...
MODULE_DESCRIPTION("Driver to write to a LCD text display - 4bit mode");
...
/* LCD char buffer */
static char lcd_buffer[17]; /* Included null character */
...
void lcd_send_byte(char data) {
	...
	x = ((data) & 0xF0); /* Send upper nibble */

	for(i=0; i<4; i++)
		gpio_set_value(gpios[i+2], ((x) & (1<<(i+4))) >> (i+4));
	lcd_enable(); 
	msleep(5);

	y =  ((data) & 0x0F);  /* Send lower nibble */

	for(i=0; i<4; i++)
		gpio_set_value(gpios[i+2], ((y) & (1<<i)) >> i);
	lcd_enable();
	msleep(5);
}
...
/**
 *@brief send a data to the line 1 or 2 in the LCD function
 *
 *@param row
 */
void lcd_string_line(int row) {
...
static ssize_t driver_write(struct file *File, const char *user_buffer, size_t count, loff_t *offs) {
	...
	/* Set the new data at line1 */
	lcd_command(0x80);

	for(i=0; i<to_copy; i++)
		lcd_data(lcd_buffer[i]);

	/* Set the new data at line2 
	lcd_command(0xC0);
...
static int __init ModuleInit(void) {
	int i;
	char *names[] = {"ENABLE_PIN", "REGISTER_SELECT", "DATA_PIN4", "DATA_PIN5", "DATA_PIN6", "DATA_PIN7"};
	printk("Hello, Kernel!\n");
	...
	lcd_command(0x02);      /* Send for initialization of LCD with nibble */

	lcd_command(0x28);	/* Set the display for 4 bit data interface for 2 lines */
